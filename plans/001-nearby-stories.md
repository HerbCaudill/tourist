...
Send location, accuracy, search bounds, current date, and the editorial instructions to a Tourist backend. Use Herb’s existing `google-maps` skill as the starting point for place resolution: geocode a typed fallback, reverse-geocode browser coordinates, discover nearby places, and fetch details by place ID. Existing API access passed live Edinburgh probes for Geocoding, Places API New nearby search, and Maps Static API on 9 September 2026. Use provider-supplied place IDs and coordinates as geographic evidence; the research runner then finds and supports the stories connected to those places. Place metadata alone does not establish a historical claim. Keep this behind a narrow server-side places adapter, with explicit failures and bounded requests, rather than making the model guess locations.
...
Check the existing request/response endpoint against Vercel’s applicable request limits and mobile connection behavior. Long waits are acceptable, but dropped requests are not. Use persisted research jobs with status polling on Cloudflare. The restricted runner permits ten-minute runs, a discovery may need multiple radius passes, and the personal Vercel Pro team has an 800-second per-request ceiling. Persist job payloads/results encrypted with bounded expiry, accept idempotent client-generated job IDs, and make reconnection independent of the original HTTP request. Keep this infrastructure independently reviewed. Measure latency to inform progress behavior and timeout handling, not to impose a speed target. Streaming or changes to `codex-cloud` should follow a demonstrated need.

Google Maps API access is distinct from the Google Workspace credentials used by the existing runner. Prefer resolving places in the Tourist backend and passing bounded place records into restricted web research; keep the Maps API key server-side and out of client responses. Read the existing `google-maps` skill and its API reference before choosing between its CLI/MCP tools and direct API calls. Verify the provider’s current display, attribution, and retention requirements before combining returned content with the prototype’s OpenStreetMap tiles or local cache. Use Google Static Maps through the server for the live small map, preserving its Ledger size and numbered markers. Google-derived coordinates and IDs can supply transient geographic anchors; source story labels and text independently rather than displaying raw Google place names or formatted addresses beside the map. Keep Maps attribution visible. Do not archive raw candidate responses or map images, and expire Google-derived coordinates within 30 days. These choices support the applicable map-display constraints while the account billing jurisdiction remains unconfirmed. See Google’s [Places policy](https://developers.google.com/maps/documentation/places/web-service/policies), [service-specific terms](https://cloud.google.com/maps-platform/terms/maps-service-terms), and [EEA terms](https://cloud.google.com/terms/maps-platform/eea/maps-service-terms).
...
Keep a small set of research evaluation locations: the motivating Edinburgh churchyard, a dense historic area, a quiet residential block, and a location with a recent event. Following Herb's September 9 editorial direction, judge these as a fun walking companion: interesting stories, variety, a plausible connection to the nearby place, readable prose, useful links, and reasonable response time. A local-history or tourism page is sufficient; do not require scholarly sources, multiple-source corroboration, or an audit of every date. Legends and disputed accounts are welcome with a natural label. Spot-check for obvious invention or a wrong place, rather than treating minor historical imprecision as a release blocker. Do not hard-code the poet into the application to pass the benchmark.
...
- Do live research evaluations yield adequate source-supported stories from the Google-supplied geographic candidates? Enabled API access is verified; story quality still needs the live checkpoint.
- Persisted jobs are required for request lifetime independence. Verify the queued implementation under retry, duplicate delivery, expiry, and mobile reconnection before release.
