...
Nearby Search (New) requests only IDs, display names and coordinates for up to twelve geographic candidates. Names are temporary identification clues and appear with Google Maps attribution in the in-memory progress feed; the generation prompt uses model knowledge for place labels and stories. The completed discovery never contains provider descriptions, names, addresses, photos, reviews, or the candidate list. Recovery tickets do not contain candidate names. Nearby responses already establish place IDs and coordinates, so extra Place Details calls would duplicate that data. Reverse geocoding is unnecessary for coordinate-based GPS labels; typed fallback uses geocoding and preserves the user's label.
