...
Create the initial UUID once per user action and retain it through connection retries. Job IDs are deterministically derived from that UUID and the bounded operation/context. If a submission succeeds but its response is lost, replaying the initial request reuses that job. Radius expansion also reuses deterministic IDs. Ordinary polling never starts the original job again. The initial encrypted ticket is also stored as the runner job’s bounded opaque `context` field. Replayed submissions read that original context before calling Google, so a nearby-search outage or changed orientation clues cannot change the origin or radius of already running research. Concurrent duplicate submissions recover the first stored context even when their orientation queries differ. Completed model output is geocoded during polling; a retry may repeat those bounded site lookups, but never regenerates the stories. A geocoding outage remains a retryable error.

Discovery starts at 200 meters. The server alone authorizes 500 meters and then 1 kilometer after a valid result leaves no geographically supported story. One accepted story stops expansion; a quota never triggers it. Malformed output, unsupported source URL schemes, and upstream failures are errors, not empty discoveries. Nearby places orient the model; they do not constrain eligible story sites. Each draft supplies a specific present-day address, intersection, or identifiable site in `locationQuery`. The server geocodes it independently, rejects partial matches and broad street/neighborhood results, and drops unmatched or out-of-radius sites. The public story receives the resolved provider ID and coordinates, with server-calculated straight-line distance and bearing; the model cannot set these fields. Location resolution does not verify the historical account or its association with that address. Three empty searches return a completed discovery with no stories and radius 1000.
...
Nearby Search (New) runs two concurrent requests per radius, each asking for up to twelve places ranked by distance, with only IDs, display names, formatted addresses and coordinates. The first searches landmarks, attractions, parks, religious/historical sites and cultural venues. The second adds public institutions, community spaces, schools, universities, shops, cafés, pubs and restaurants. Landmark results take priority; distinct broader results fill the remaining slots to a maximum of twelve, deduplicated by provider ID and then sorted by distance for display and model context. Either search can supply context if the other fails; failure of both remains an error. Sites outside the radius are removed before filling the list. Stories remain independent of this orientation list. The model receives nearby place names only. Provider IDs, addresses and coordinates are omitted from the prompt. Names are temporary identification clues and appear in the in-memory progress feed; the generation prompt uses model knowledge for place labels and stories. The completed discovery never contains provider descriptions, names, addresses, photos, reviews, the orientation list, or the geocoding query. Recovery tickets contain the origin and radius, not the orientation list. Story sites are geocoded independently, including addresses and intersections absent from Nearby Search; accepted result types are street addresses, premises/subpremises, intersections, points of interest, and establishments. No Place Details calls are required. GPS locations are reverse-geocoded to a readable location for the model; typed locations preserve the user's label. This description is used transiently in the prompt.
...
Use Vercel runtime logs to find `research_timing`, then match the job ID in codex-cloud's Cloudflare logs. The worker measures queue wait, the direct OpenAI request (`openai` phase), and per-attempt completion/persistence. `research_api_trace` records numeric token usage without prompts or results. Attempt numbers distinguish retries. A request that expands its radius has another job ID under the same request ID. No timing metadata is added to the browser API.

## Generation settings

Tourist uses the direct OpenAI Responses API with `gpt-6-astra` at `low` reasoning effort, standard processing, no tools, and a 6,000-token output ceiling including reasoning. Each existing Cloudflare job object schedules an immediate durable alarm to run its API call independently. New requests do not enter the shared queue or launch Codex CLI or a container. The existing encrypted job store, idempotent submission, recovery tickets, and polling remain in use. `OPENAI_API_KEY` is a Cloudflare Worker secret; Vercel continues to use the existing private job endpoint credentials.

Discovery and chat answer from model knowledge and supplied context; stories may have empty source arrays, and chat may reuse links already supplied in context. Google still resolves locations and nearby geographic anchors. The model must not invent citations or claim current verification. Requests set `store: false`; application results remain encrypted in the existing job store for up to 24 hours. API generation has a three-minute timeout. An uncertain connection failure does not automatically regenerate the answer. Model usage is billed to the OpenAI API account separately from the Codex subscription.
