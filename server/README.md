...
Create the initial UUID once per user action and retain it through connection retries. Job IDs are deterministically derived from that UUID and the bounded operation/context. If a submission succeeds but its response is lost, replaying the initial request reuses that job. Radius expansion also reuses deterministic IDs. Ordinary polling never starts the original job again. Concurrent duplicate submissions are additionally deduplicated by the runner.
